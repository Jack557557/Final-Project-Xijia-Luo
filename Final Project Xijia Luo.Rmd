...
# Data information and input

This data set is about League of Legends competitive matches between 2015-2017. The matches include the NALCS, EULCS, LCK, LMS, and CBLoL leagues as well as the World Championship and Mid-Season Invitational tournaments.
```{r}
read.csv
```
...
$Pr(\text{Penny Win } | \text{ Spinner A}) = 2/3$
...
Yes, because the $Pr(\text{Penny Win } | \text{ Spinner A}) = 2/3$, while $Pr(\text{Penny Win } | \text{ Spinner B}) = 1/2$. Spinner A is higher.
...
2. Becuase 4/6 * 7 = 14/3 = 4.666667. For lower bound, 2/6 * 7 = 2.333333 close to 2

<br>

**State new a UPPER bound for your estimate for Spinner A, and explain your reasoning:**

7. Becuase 4/6 * 7 = 14/3 = 4.666667. For lower bound, 6/6 * 7 = 7
...
```{r}
spinner <- function(nsim){
SpinnerA <- c("Penny", "Penny", "Penny", "Nickel", "Nickel", "Nickel")
spin <- c()
penny <- 0
nickel <- 0
pennyWins <- 0
for(i in 1:nsim){
while(TRUE){
spin <- sample(SpinnerA, 1, replace = TRUE, prob = NULL)
sample = spin
if(sample == "Penny"){
penny = penny +1 }
else {
